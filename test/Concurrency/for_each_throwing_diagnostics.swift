...
func defaultCase(_ seq: ThrowingSeq) async {
  for await _ in seq { }
  // expected-error@-1 {{errors thrown from 'for-in' loop are not handled}} {{7-7=try }}
}
...
func rethrowsContext(
  ...
  try body()
  for await _ in seq { }
  // expected-error@-1 {{errors thrown from 'for-in' loop are not handled; a function declared 'rethrows' may only throw if its parameter does}} {{7-7=try }}
}
...
func nonExhaustiveCatch(_ seq: TypedThrowingSeq) async {
  do {
    for await _ in seq { }
    // expected-error@-1 {{errors thrown from 'for-in' loop are not handled because the enclosing catch is not exhaustive}} {{9-9=try }}
...
func missingTryHandled(_ seq: ThrowingSeq) async throws {
  for await _ in seq { }
  // expected-error@-1 {{'for-in' loop can throw, but is not marked with 'try'}} {{7-7=try }}
}